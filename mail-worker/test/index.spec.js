...
describe('temporary inbox API compatibility', () => {
	...
	it('manages private Outlook accounts, groups, tags, and Graph inbox synchronization', async () => {
		...
		try {
			const unauthorizedOAuthStart = await request('/api/outlookAccount/oauth/start', await authOptions(1, [], 'POST')); expect((await unauthorizedOAuthStart.json()).code).toBe(403);
			const imported = await request('/api/outlookAccount/import', await authOptions(1, ['outlook-account:add'], 'POST', { rows: 'outlook@example.com----password----client-id----refresh-token\nbroken-row' }));
			...
			const group = (await (await request('/api/outlookGroup/add', await authOptions(1, ['outlook-group:add'], 'POST', { name: 'Work', sort: 10 }))).json()).data;
			const earlierGroup = (await (await request('/api/outlookGroup/add', await authOptions(1, ['outlook-group:add'], 'POST', { name: 'Personal', sort: 0 }))).json()).data;
			expect((await (await request('/api/outlookGroup/list', await authOptions(1, ['outlook-group:query']))).json()).data.list.map(item => item.outlookGroupId)).toEqual([earlierGroup.outlookGroupId, group.outlookGroupId]);
			...
			const emailRows = await env.db.prepare(`SELECT subject, account_id FROM email WHERE user_id = 1`).all(); expect(emailRows.results).toEqual(expect.arrayContaining([expect.objectContaining({ subject: 'Verification 123456' }), expect.objectContaining({ subject: 'Junk message' })])); const attachments = await env.db.prepare(`SELECT filename FROM attachments WHERE email_id IN (SELECT email_id FROM email)`).all(); expect(attachments.results[0].filename).toBe('proof.txt');
			...
			const outlookMessages = await request(`/api/email/list?accountId=${localAccount.account_id}&allReceive=0&emailId=0&timeSort=0&size=50&type=0&outlook=1&outlookFolder=inbox`, await authOptions(1)); expect((await outlookMessages.json()).data).toMatchObject({ total: 1, list: [expect.objectContaining({ subject: 'Verification 123456' })] }); const junkMessages = await request(`/api/email/list?accountId=${localAccount.account_id}&allReceive=0&emailId=0&timeSort=0&size=50&type=0&outlook=1&outlookFolder=junkemail`, await authOptions(1)); expect((await junkMessages.json()).data).toMatchObject({ total: 1, list: [expect.objectContaining({ subject: 'Junk message' })] });
			...
			await env.db.prepare(`INSERT INTO role (role_id, name, key) VALUES (2, 'sync-only', '')`).run();
			await env.db.prepare(`INSERT INTO user (user_id, email, type) VALUES (3, 'sync-only@example.com', 2)`).run();
			await env.db.prepare(`INSERT INTO account (account_id, email, name, user_id) VALUES (10, 'sync-only@example.com', 'sync-only', 3)`).run();
			await env.db.prepare(`INSERT INTO outlook_connection(user_id, provider_email, client_id, refresh_token_ciphertext) VALUES (3, 'sync-only@example.com', 'client-id', 'refresh-token')`).run();
			const syncOnlyConnection = await env.db.prepare(`SELECT outlook_connection_id FROM outlook_connection WHERE user_id = 3`).first();
			await env.db.prepare(`INSERT INTO outlook_account(user_id, email, client_id, client_secret_ciphertext, refresh_token_ciphertext, outlook_connection_id) VALUES (3, 'sync-only@example.com', 'client-id', 'client-secret', 'refresh-token', ?)`).bind(syncOnlyConnection.outlook_connection_id).run();
			const syncOnlyAccountList = await request('/api/outlookAccount/list?limit=50&offset=0', await authOptions(3, ['outlook-sync:run'])); expect((await syncOnlyAccountList.json()).data.list).toEqual([expect.objectContaining({ email: 'sync-only@example.com', accountId: 10 })]);
			const syncOnlyGroups = await request('/api/outlookGroup/list', await authOptions(3, ['outlook-sync:run'])); expect(syncOnlyGroups.status).toBe(200);
