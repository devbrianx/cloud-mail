...
describe('temporary inbox API compatibility', () => {
	...
	it('manages private Outlook accounts, groups, tags, and Graph inbox synchronization', async () => {
		...
		try {
			...
			const synced = await request('/api/outlookSync/run', await authOptions(1, ['outlook:query'], 'POST', { outlookAccountId: account.outlookAccountId })); expect((await synced.json()).data).toMatchObject({ isOutlook: true, received: 2 });
