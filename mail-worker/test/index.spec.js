...
function microsoftMock() {
	return vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init = {}) => {
		...
		if (url.includes('/token')) {
			...
			if (body.includes('refresh_token=missing-secret-refresh')) return new Response(JSON.stringify({ error: 'invalid_request', error_description: "AADSTS7000218: The request body must contain the following parameter: 'client_secret'.\r\nTrace ID: test" }), { status: 400 });
			if (body.includes('refresh_token=no-description-refresh')) return new Response(JSON.stringify({ error: 'invalid_request' }), { status: 400 });
			if (body.includes('refresh_token=default-scope-refresh') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2F.default')) return new Response(JSON.stringify({ access_token: 'default-scope-token', refresh_token: 'default-scope-refresh' }), { status: 200 });
			if (body.includes('refresh_token=mail-read-fallback-refresh') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2F.default')) return new Response(JSON.stringify({ error: 'invalid_request', error_description: 'AADSTS90023: No applicable permissions were found for this user.' }), { status: 400 });
			if (body.includes('refresh_token=mail-read-fallback-refresh') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2FMail.Read')) return new Response(JSON.stringify({ access_token: 'mail-read-token', refresh_token: 'mail-read-fallback-refresh' }), { status: 200 });
			if (body.includes('refresh_token=invalid-grant-refresh')) return new Response(JSON.stringify({ error: 'invalid_grant', error_description: 'AADSTS70000: Scope is unauthorized.' }), { status: 400 });
			if (body.includes('refresh_token=idx-refresh') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2F.default')) return new Response(JSON.stringify({ access_token: 'idx-primary-token', refresh_token: 'idx-refresh' }), { status: 200 });
			if (body.includes('refresh_token=idx-refresh') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2FMail.Read')) return new Response(JSON.stringify({ access_token: 'idx-mail-read-token', refresh_token: 'idx-rotated-refresh' }), { status: 200 });
			if (body.includes('grant_type=refresh_token') && body.includes('scope=https%3A%2F%2Fgraph.microsoft.com%2F.default')) return new Response(JSON.stringify({ error: 'invalid_request', error_description: 'AADSTS90023: No applicable permissions were found for this user.' }), { status: 400 });
			return new Response(JSON.stringify({ access_token: body.includes('refresh_token=graph-error-refresh') ? 'graph-error-token' : 'access-token', refresh_token: body.includes('refresh_token=graph-error-refresh') ? 'graph-error-refresh' : 'rotated-refresh' }), { status: 200 });
		}
		if (url.endsWith('/me')) return new Response(JSON.stringify({ mail: 'outlook@example.com' }), { status: 200 });
		if (url.includes('/messages/delta') && new Headers(init.headers).get('authorization') === 'Bearer graph-error-token') return new Response(JSON.stringify({ error: { code: 'ErrorAccessDenied', message: 'Access to OData is disabled.' } }), { status: 403 });
		if (url.includes('/messages/delta') && new Headers(init.headers).get('authorization') === 'Bearer idx-primary-token') return new Response(JSON.stringify({ error: { code: 'InvalidAuthenticationToken', message: 'IDX14100: JWT is not well formed.' } }), { status: 401 });
...
describe('temporary inbox API compatibility', () => {
	...
	it('manages private Outlook accounts, groups, tags, and Graph inbox synchronization', async () => {
		...
		try {
			...
			const detailedFailure = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'secret@example.com----password----client-id----missing-secret-refresh' })); expect((await detailedFailure.json()).data).toEqual({ imported: [], failed: [{ line: 1, reason: "Microsoft token refresh failed (invalid_request): AADSTS7000218: The request body must contain the following parameter: 'client_secret'. Trace ID: test" }] });
			const genericFailure = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'generic@example.com----password----client-id----no-description-refresh' })); expect((await genericFailure.json()).data).toEqual({ imported: [], failed: [{ line: 1, reason: 'Microsoft token refresh failed (invalid_request)' }] });
			const invalidGrantFailure = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'grant@example.com----password----client-id----invalid-grant-refresh' })); expect((await invalidGrantFailure.json()).data.failed[0].reason).toBe('Microsoft token refresh failed (invalid_grant): AADSTS70000: Scope is unauthorized.'); expect(mock.mock.calls.filter(([url, init]) => String(url).includes('/token') && String(init?.body).includes('refresh_token=invalid-grant-refresh'))).toHaveLength(1);
			await env.db.prepare(`UPDATE outlook_connection SET client_secret_ciphertext = 'encrypted-client-secret' WHERE user_id = 1 AND provider_email = 'outlook@example.com'`).run();
			const exportedAll = await request('/api/outlookAccount/export', await authOptions(1, ['outlook:query'], 'POST', {})); const exportedAllRows = (await exportedAll.json()).data.rows; expect(exportedAllRows).toBe('outlook@example.com----x----client-id----rotated-refresh'); expect(exportedAllRows).not.toContain('encrypted-client-secret');
			const importedAccount = (await env.db.prepare(`SELECT outlook_account_id FROM outlook_account WHERE user_id = 1 AND email = 'outlook@example.com'`).first()).outlook_account_id;
			const exportedSelected = await request('/api/outlookAccount/export', await authOptions(1, ['outlook:query'], 'POST', { outlookAccountIds: [importedAccount] })); expect((await exportedSelected.json()).data.rows).toBe('outlook@example.com----x----client-id----rotated-refresh'); await env.db.prepare(`UPDATE outlook_connection SET client_secret_ciphertext = '' WHERE user_id = 1 AND provider_email = 'outlook@example.com'`).run();
			await env.db.prepare(`INSERT INTO outlook_connection(user_id, provider_email, client_id, refresh_token_ciphertext) VALUES (3, 'foreign-export@example.com', 'client-id', 'refresh-token')`).run(); const foreignExportConnection = await env.db.prepare(`SELECT outlook_connection_id FROM outlook_connection WHERE user_id = 3`).first(); const foreignExportAccount = (await env.db.prepare(`INSERT INTO outlook_account(user_id, email, client_id, client_secret_ciphertext, refresh_token_ciphertext, outlook_connection_id) VALUES (3, 'foreign-export@example.com', 'client-id', '', 'refresh-token', ?)`).bind(foreignExportConnection.outlook_connection_id).run()).meta.last_row_id;
			const foreignExport = await request('/api/outlookAccount/export', await authOptions(1, ['outlook:query'], 'POST', { outlookAccountIds: [foreignExportAccount] })); expect((await foreignExport.json()).code).toBe(404); const emptyExport = await request('/api/outlookAccount/export', await authOptions(2, ['outlook:query'], 'POST', {})); expect((await emptyExport.json()).data.rows).toBe(''); await env.db.prepare(`UPDATE outlook_account SET is_del = 1 WHERE outlook_account_id = ?`).bind(foreignExportAccount).run(); await env.db.prepare(`UPDATE outlook_connection SET is_del = 1 WHERE outlook_connection_id = ?`).bind(foreignExportConnection.outlook_connection_id).run();
			const idxImport = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'idx@example.com----password----client-id----idx-refresh' })); const idxAccount = (await idxImport.json()).data.imported[0]; const idxSync = await request('/api/outlookSync/run', await authOptions(1, ['outlook:query'], 'POST', { outlookAccountId: idxAccount.outlookAccountId })); expect((await idxSync.json()).data).toMatchObject({ isOutlook: true }); expect(mock.mock.calls.filter(([url, init]) => String(url).includes('/token') && String(init?.body).includes('refresh_token=idx-refresh') && String(init?.body).includes('scope=https%3A%2F%2Fgraph.microsoft.com%2FMail.Read'))).toHaveLength(1); await request(`/api/outlookAccount/delete?outlookAccountId=${idxAccount.outlookAccountId}`, await authOptions(1, ['outlook:query'], 'DELETE'));
			...
			const defaultScopeImport = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'default-scope@example.com----password----client-id----default-scope-refresh' })); const defaultScopeAccount = (await defaultScopeImport.json()).data.imported[0]; expect(defaultScopeAccount).toBeDefined();
			const fallbackScopeImport = await request('/api/outlookAccount/import', await authOptions(1, ['outlook:query'], 'POST', { rows: 'mail-read@example.com----password----client-id----mail-read-fallback-refresh' })); const fallbackScopeAccount = (await fallbackScopeImport.json()).data.imported[0]; expect(fallbackScopeAccount).toBeDefined();
			const refreshExchanges = mock.mock.calls.filter(([url, init]) => String(url).includes('/token') && String(init?.body).includes('grant_type=refresh_token')); expect(refreshExchanges.some(([, init]) => String(init?.body).includes('refresh_token=default-scope-refresh') && String(init?.body).includes('scope=https%3A%2F%2Fgraph.microsoft.com%2F.default'))).toBe(true); expect(refreshExchanges.some(([, init]) => String(init?.body).includes('refresh_token=mail-read-fallback-refresh') && String(init?.body).includes('scope=https%3A%2F%2Fgraph.microsoft.com%2FMail.Read'))).toBe(true);
			await request(`/api/outlookAccount/delete?outlookAccountId=${defaultScopeAccount.outlookAccountId}`, await authOptions(1, ['outlook:query'], 'DELETE')); await request(`/api/outlookAccount/delete?outlookAccountId=${fallbackScopeAccount.outlookAccountId}`, await authOptions(1, ['outlook:query'], 'DELETE'));
