  ...
  <el-scrollbar class="scroll">
    <div>
      ...
      <el-menu :collapse="false" text-color="#fff" active-text-color="#fff" style="margin-top: 10px">
        ...
        <el-menu-item @click="router.push({name: 'temporary-identity'})" index="temporary-identity"
        ...
        <div class="manage-title" v-perm="['all-email:query','user:query','role:query','setting:query','analysis:query','reg-key:query','outlook:query']">
          <div>{{$t('manage')}}</div>
        </div>
        <el-sub-menu index="outlook-mailbox-management" v-perm="'outlook:query'">
          <template #title>
            <Icon icon="hugeicons:mailbox-01" width="21" />
            <span class="menu-name" style="margin-left: 20px">{{$t('outlookMailboxManagement')}}</span>
          </template>
          <el-menu-item class="outlook-submenu-item" v-perm="'outlook:query'" @click="router.push({name: 'outlook-accounts'})" index="outlook-accounts" :class="route.meta.name === 'outlook-accounts' ? 'choose-item' : ''">
            <Icon icon="solar:users-group-rounded-outline" width="20" />
            <span class="menu-name" style="margin-left: 20px">{{$t('outlookAccountPool')}}</span>
          </el-menu-item>
          <el-menu-item class="outlook-submenu-item" v-perm="'outlook:query'" @click="router.push({name: 'outlook-groups'})" index="outlook-groups" :class="route.meta.name === 'outlook-groups' ? 'choose-item' : ''">
          ...
          <el-menu-item class="outlook-submenu-item" v-perm="'outlook:query'" @click="router.push({name: 'outlook-service'})" index="outlook-service" :class="route.meta.name === 'outlook-service' ? 'choose-item' : ''">
