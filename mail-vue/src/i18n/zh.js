const zh = {
    ...
    ,apiGuideApiKeyAuth: 'API 密钥操作'
    ,apiGuideTokenAuth: '临时 token 操作'
    ,apiGuideScopes: '权限范围'
    ,apiGuideCreateAccount: '创建临时邮箱'
    ,apiGuideCreateAccountDesc: '需要 inboxes:write。请把示例域名替换为已启用的 API 域名，并使用未被占用的前缀。'
    ,apiGuideCreateResponse: '成功响应的 data 包含 id、address、expiresAt，以及仅在创建时返回的 token。'
    ,apiGuideCurrentAccount: '查看当前临时邮箱'
    ,apiGuideCurrentAccountDesc: '使用创建时返回的 token。临时 token 只能访问其绑定的邮箱。'
    ,apiGuideListMessages: '查询邮件'
    ,apiGuideListMessagesDesc: '需要 messages:read。使用 API 密钥查询邮件时必须传入 address，且只能访问该密钥创建的邮箱。'
    ,apiGuideRestrictions: '适用范围'
    ,apiGuideRestrictionsDesc: 'Webhook、持久邮箱、邮件发送和 DNS 自动配置不属于此 API。'
}
export default zh
